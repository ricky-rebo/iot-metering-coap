...
public class PutMeterStateClient {
    private static final Logger logger = LoggerFactory.getLogger(PutMeterStateClient.class);
    ...
    private static final Boolean VALUE_TO_SEND = false;

    public static void main (String[] args) {
        CoapClient client = new CoapClient(TARGET_URL);

        try {
            CoapResponse response = client.advanced(new Request(CoAP.Code.PUT) {{
                setConfirmable(true);
                setPayload(Boolean.toString(VALUE_TO_SEND));
            }});
